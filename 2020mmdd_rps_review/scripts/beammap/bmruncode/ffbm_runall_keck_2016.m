function ffbm_runall_keck_2016(bmnum,demod,fullmap,winmap,winmaplarge,...
    html,plotmap)
% ffbm_runall_keck_2016(bmnum,demod,fullmap,winmap,winmaplarge,html,plotmap)
%
...
% >> ffbm_runall_keck_2016(1,1,1,1,0,0); % For bm run 1 in 2015
%    Then modify the .csv field 'filename' with the timestamp found in
%    beammaps/maps (can do this with make_expt_year_bmcsv.m)
% >> ffbm_runall_keck_2016(1,0,0,0,1,1); % And the plots should be made,
%    with the HTML already in there!
% >> for ii = 2:??; ffbm_runall_keck_2016(ii,2,0,0,0,0); end; % Farm demods
...
switch demod
  ...
  case 2
    ...
    tmpfile = ['farmfiles/demod/k16demod_bm' sprintf('%02i',bmnum) '_' s];
    cmd = 'bm_demod(bmopt)';
...
switch fullmap
  ...
  case 2
    ...
    tmpfile = ['farmfiles/fullmap/k16fullmap_bm' sprintf('%02i',bmnum) '_' s];
    cmd = 'bm_makemap(bmopt)';
    farmit(tmpfile,cmd,'var',{'bmopt'},'queue',...
	'general,serial_requeue,itc_cluster,regal','mem',100000,'maxtime',1440);
end

% Initial source position - change when we switch
bmopt.source_az = 0;
bmopt.source_el = 4.25;
if bmnum > 29
  bmopt.source_az = 0.1;
  bmopt.source_el = 5.0;
end
...
switch winmap
  ...
  case 2
    ...
    tmpfile = ['farmfiles/winmap/k16winmap_bm' sprintf('%02i',bmnum) '_' s];
    cmd = 'bm_makemap(bmopt)';
    farmit(tmpfile,cmd,'var',{'bmopt'},'queue',...
	'general,serial_requeue,itc_cluster,regal','mem',100000,'maxtime',1440);
end
...
switch plotmap
 ...
 case 2
  ...
  tmpfile = ['farmfiles/plotter/k16plotter_bm' sprintf('%02i',bmnum) '_' s];
