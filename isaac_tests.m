...
% build a 9-color lookup table for color coding points
c=lines(10);
ang = [-2 -1.5 -1 -0.5 0 0.5 1 1.5 2];
indices = 1:9;
col_dict = dictionary(ang, indices);
...
for measind = 1:length(meas_names)
    ...
    for schind = 1:length(schnums)
        ...
        dP = fd.phi(idx)-fd.phi_isaac(idx);
        try
            plot(times,dP,'.','Color',c(col_dict(offs(schind)),:),'MarkerSize',14)
        catch
            plot(times,dP,'.','Color',c(10,:),'MarkerSize',14)
        end
        %plot(repmat(offs(schind),1,length(idx)),fd.tilt(idx),'x','Color',cmlines(schind,:),'MarkerSize',10)
        %plot(repmat(offs(schind),1,length(idx)),fd.istilt(idx),'^','Color',cmlines(schind,:),'MarkerSize',10)
    ...
    %plot(offs_all,dp_all,'Color',cmlines(2,:))
    plot(offs_all,dp_all,'Color',"#808080")
    grid on
    %xlim([-1 1]*2.5)
    if nanmedian(dp_all)<1 && nanmedian(dp_all)>-1
        ylim(lims);
...
xlabel('Time')
title_str = sprintf('Noise Check at 1m, Schedule %i',schind);
title(title_str)
grid on
...
prefix = {...
    ...
    'isaac_cal_jig_july23_with_homing_stabtest_aug10night_1m';...
    };
...
labs = {...;
    ...
    'On Jig, Homing, Dist = 43", Small Horn Flipped, RPS align +0.0deg, Stab Test Round 7';... 250
    };
