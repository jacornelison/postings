function ffbm_runall_b3_2016(bmnum,demod,fullmap,winmap,winmaplarge,...
                             html,plotmap)
% ffbm_runall_b3_2016(bmnum,demod,fullmap,winmap,winmaplarge,...
%                     html,plotmap)
%
...
% bmopt.demodshift = []; % Defaults based on dates in bm_demod

% Pointing model hack to avoid SINGULARITY
mjd = date2mjd(datenum('20160201','yyyymmdd'));
...
switch demod
  ...
  case 2
    ...
    tmpfile = ['farmfiles/demod/b3demod_bm' sprintf('%02i',bmnum) '_' s];
    cmd = 'bm_demod(bmopt)';
...
switch fullmap
  ...
  case 2
    ...
    tmpfile = ['farmfiles/fullmap/b3fullmap_bm' sprintf('%02i',bmnum) '_' s];
    cmd = 'bm_makemap(bmopt)';
    farmit(tmpfile,cmd,'var',{'bmopt'},'queue',...
	'general,serial_requeue,itc_cluster','mem',100000,'maxtime',1440);
end

bmopt.source_az = -178;
bmopt.source_el = 3.35;
bmopt.maptype = 'window';
bmopt.dofit = 1;
bmopt.beamcen = 'obs';

% Until we figure out the pointing problem, 2 degree windows are a little
% too small for beam fitting.  Go to 3 degrees...
bmopt.winsize = 3;
...
switch winmap
  ...
  case 2
    ...
    tmpfile = ['farmfiles/winmap/b3winmap_bm' sprintf('%02i',bmnum) '_' s];
    cmd = 'bm_makemap(bmopt)';
    farmit(tmpfile,cmd,'var',{'bmopt'},'queue',...
	'general,serial_requeue,itc_cluster','mem',40000,'maxtime',1440);
end
...
switch winmaplarge
  ...
  case 2
    s = gen_stamp();
    mkdir('farmfiles/winmap')
    tmpfile = ['farmfiles/winmap/b3winmaplg_bm' sprintf('%02i',bmnum) '_' s];
    cmd = 'bm_makemap(bmopt)';
    farmit(tmpfile,cmd,'var',{'bmopt'},'queue',...
	'general,serial_requeue,itc_cluster','mem',40000,'maxtime',1440);
    bmopt.suffix = '';
