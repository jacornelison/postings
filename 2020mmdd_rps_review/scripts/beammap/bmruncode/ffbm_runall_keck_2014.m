function ffbm_runall_keck_2014(bmnum,demod,fullmap,winmap,winmaplarge,...
    html,plotmap)
% ffbm_runall_keck_2014(bmnum,demod,fullmap,winmap,winmaplarge,html,plotmap)
%
...
% >> ffbm_runall_keck_2014(1,1,1,1,0,0); % For bm run 1 in 2014
%    Then modify the .csv field 'filename' with the timestamp found in
%    beammaps/maps (can do this with make_expt_year_bmcsv.m)
% >> ffbm_runall_keck_2014(1,0,0,0,1,1); % And the plots should be made,
%    with the HTML already in there!
% >> for ii = 2:57; ffbm_runall_keck_2014(ii,2,0,0,0,0); end; % Farm demods
...
%bmopt.demodshift = []; % Nominal demodshift already in bm_demod
...
mjd = date2mjd(datenum('20140201','yyyymmdd'));
...
switch demod
  ...
  case 2
    ...
    tmpfile = ['farmfiles/demod/k14demod_bm' sprintf('%02i',bmnum) '_' s];
    cmd = 'bm_demod(bmopt)';
...
switch fullmap
  ...
  case 2
    ...
    tmpfile = ['farmfiles/fullmap/k14fullmap_bm' sprintf('%02i',bmnum) '_' s];
    cmd = 'bm_makemap(bmopt)';
    farmit(tmpfile,cmd,'var',{'bmopt'},'queue',...
	'general,serial_requeue,itc_cluster,regal','mem',80000,'maxtime',1440);
end

% For 2014, the source position is approximately the same for both mirror
% positions
bmopt.source_az = 0.3;
bmopt.source_el = 3.8;
...
switch winmap
  ...
  case 2
    ...
    tmpfile = ['farmfiles/winmap/k14winmap_bm' sprintf('%02i',bmnum) '_' s];
    cmd = 'bm_makemap(bmopt)';
    farmit(tmpfile,cmd,'var',{'bmopt'},'queue',...
	'general,serial_requeue,itc_cluster,regal','mem',40000,'maxtime',1440);
end
...
switch winmaplarge
  ...
  case 2
    s = gen_stamp();
    mkdir('farmfiles/winmap')
    tmpfile = ['farmfiles/winmap/k14winmapl_bm' sprintf('%02i',bmnum) '_' s];
    cmd = 'bm_makemap(bmopt)';
    farmit(tmpfile,cmd,'var',{'bmopt'},'queue',...
...
switch plotmap
 ...
 case 2
  ...
  tmpfile = ['farmfiles/plotter/k14plotter_bm' sprintf('%02i',bmnum) '_' s];
