function ma_posting_plots_2022mmdd()
...
postingdir = 'c:/Users/James/Documents/Github/postings/2022mmdd_mirror_analysis_3/';
datadir = fullfile(postingdir,'data','');
figdir = fullfile(postingdir,'figs','');

scriptdirs = {'z:/dev','z:/pipeline',fullfile(postingdir,'scripts','')};
cellfun(@(c) addpath(genpath(c)),scriptdirs)
...
switch ind
    case 1
    load(fullfile(datadir,'timestream_moonsch_4_scan_1.mat'))
    fname = fullfile(datadir,'horizons_moonsch_4_scan_1_geo.txt');
    fname = fullfile(datadir,'horizons_moonsch_4_scan_1_horiz_topo.txt');
    f = fopen(fname);
    ...
    index = 6;
    case 2
    load(fullfile(datadir,'timestream_moonsch_54_scan_10.mat'))
    fname = fullfile(datadir,'horizons_moonsch_54_scan_10.txt');
    f = fopen(fname);
    ...
    case 3
    load(fullfile(datadir,'timestream_moonsch_54_scan_10.mat'))
    fname = fullfile(datadir,'moon.ephem');
    f = fopen(fname);
    horz_data_sch = textscan(f,'%f%s%s%f%s%s%s','delimiter',' ','HeaderLines',9,'MultipleDelimsAsOne',true);
    fclose(f);
    index = 2;
    
    cal = [0 360/24 1];
    for valind = 2:3
        val = horz_data_sch{valind};
        val_new = NaN(size(val));
        for di = 1:length(val)
            t = split(val{di},':');
            
                s = sign(str2double(t{1}));
                val_new(di) = (str2double(t{1})+s*str2double(t{2})/60+s*str2double(t{3})/3600)*cal(valind);
            
                
            
        end
        horz_data_sch{valind} = val_new;
    end
    horz_data_sch{1} = horz_data_sch{1}+2400000.5;    
...
[hazt, helt, hpat] = horz_equat_geoc_to_gcp_horiz_topo(d,hmjd,ra,dec);
hazt = interp1(hmjd,horz_data_sch{end-1},ttrack);
helt = interp1(hmjd,horz_data_sch{end},ttrack);
...
fname = fullfile(datadir,'illum_test_moon.txt');
f = fopen(fname);
hd_moon = textscan(f,'%f%s%s%f%f%f%f%f%f%f','delimiter',',','HeaderLines',60);
fclose(f);

% MJD, , ,raapp,decapp,illum%,angdia,STO
fname = fullfile(datadir,'illum_test_sun.txt');
f = fopen(fname);
