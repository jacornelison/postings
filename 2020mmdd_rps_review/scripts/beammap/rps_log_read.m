...
for ii=1:length(lf)
  ...
  while ~feof(f)
    ...
    % 2. Command to increment offset.
    pattern = 'offset/add.*az=(-?[0-9]+:[0-9]+:[0-9]+)';
    tmp = regexp(logline, pattern, 'tokens');
    if ~isempty(tmp)
      % Update offset value.
      az_ofs = az_ofs + dmsstr(tmp);
    end
    ...
    if ~in_sched
      ...
      if ~isempty(tmp)
		...
		%Do the same for az offset
		if length(sch{end}.scans) > 0
          scanaz = zeros(size(sch{end}.scans));
          for jj=1:length(scanaz)
            scanaz(jj) = sch{end}.scans(jj).az_ofs;
          end
          sch{end}.az_ofs = mode(scanaz);
        end
...
for ii=1:length(sch)
  ...
  for jj=1:length(sch{ii}.scans)
	...
	az_ofs(jj) = sch{ii}.scans(jj).az_ofs;
