function ffbm_runall_keck_2012(bmnum,demod,fullmap,winmap,winmaplarge,...
    html,plotmap)
% ffbm_runall_keck_2012(bmnum,demod,fullmap,winmap,winmaplarge,html,plotmap)
%
...
% >> ffbm_runall_keck_2012(1,1,1,1,0,0); % For bm run 1 in 2015
%    Then modify the .csv field 'filename' with the timestamp found in
%    beammaps/maps (can do this with make_expt_year_bmcsv.m)
% >> ffbm_runall_keck_2012(1,0,0,0,1,1); % And the plots should be made,
%    with the HTML already in there!
% >> for ii = 2:??; ffbm_runall_keck_2012(ii,2,0,0,0,0); end; % Farm demods
...
mjd = date2mjd(datenum('20120201','yyyymmdd'));
...
switch demod
  ...
  case 2
    ...
    tmpfile = ['farmfiles/demod/k12demod_bm' sprintf('%02i',bmnum) '_' s];
    cmd = 'bm_demod(bmopt)';
...
switch fullmap
  ...
  case 2
    ...
    tmpfile = ['farmfiles/fullmap/k12fullmap_bm' sprintf('%02i',bmnum) '_' s];
...
switch winmap
  ...
  case 2
    ...
    tmpfile = ['farmfiles/winmap/k12winmap_bm' sprintf('%02i',bmnum) '_' s];
    cmd = 'bm_makemap(bmopt)';
    farmit(tmpfile,cmd,'var',{'bmopt'},'queue',...
	'general,serial_requeue,itc_cluster,regal','mem',100000,'maxtime',1440);
end
...
switch winmaplarge
  ...
  case 2
    s = gen_stamp();
    mkdir('farmfiles/winmap')
    tmpfile = ['farmfiles/winmap/k12winmapl_bm' sprintf('%02i',bmnum) '_' s];
    cmd = 'bm_makemap(bmopt)';
    farmit(tmpfile,cmd,'var',{'bmopt'},'queue',...
	'general,serial_requeue,itc_cluster,regal','mem',100000,'maxtime',1440);
    bmopt.suffix = '';
...
switch plotmap
 ...
 case 2
  ...
  tmpfile = ['farmfiles/plotter/k12plotter_bm' sprintf('%02i',bmnum) '_' s];
