function hh = errorbar2(x, y, l,u,symbol,tee)
% Slight hack from errorbar to supress the horizontal lines
% at the top and bottom of the bar.
%
...
%   $Revision: 1.1.2.1.2.2 $  $Date: 2014/07/03 20:29:04 $
...
%tee = (max(x(:))-min(x(:)))*tee/100;  % make tee .02 x-distance for error bars
tee=0;
xl = x - tee;
xr = x + tee;
ytop = y + u;
...
symbol = [mark col ls]; % Use marker only on data part
