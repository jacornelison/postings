function [ fd ] = rps_analysis_plots(dirname,figdir, rerun, overwrite, nocut)
...
if ~nocut
    ...
    dk = unique(fd.dk);
    cut_sch = [08 06 07 07 07 07 08 09 09 05 06 06 04 04 05 05 05 09];
    cut_row = [11 14 18 02 11 17 17 04 11 11 13 04 17 09 07 10 12 15];
    ...
    cut_ind = ...
        ...
        abs(fd.aparam(:,3))>0.05 |... % High miscollimation indicates poor source stability
        abs(fd.aparam(:,4))>0.05 |...
...
for j = 1:length(dk)
    ...
    a(~ind2) = NaN;
    hold on
    plot_tiles(a,p,'clim',[-2 2],'fig',fig,'clab','degrees')
...
for j = 1:length(dk)
    ...
    a(~ind2) = NaN;
    hold on
    plot_tiles(a,p,'clim',[-2 2],'fig',fig,'clab','degrees','pair','diff')
    %
    saveas(fig,pname,'png')
end
...
for i = 1:2
    ...
    for j = 1:length(dk)
        ...
        if i==1
            ...
            pname = [figdir figfold sprintf('hist_final_a_%03i',floor(dk(j)-1.25))];
            C = 0-2.5+1.5;
            disp(pname)
        else
            ind = ismember(ch(:,1),cutchans_b);
            pname = [figdir figfold sprintf('hist_final_b_%03i',floor(dk(j)-1.25))];
            C = 90-2.5+1.5;
            disp(pname)
        end
...
for i = 1:2
    ...
    if i==1
        ind = ismember(ch(:,1),cutchans_a);
        pname = [figdir figfold 'hist_final_a_all'];
        C = 0-2.5+1.5;
    else
        ind = ismember(ch(:,1),cutchans_b); %& ~any(isnan(a),2) ;%& any(abs(a)<4,2);
        pname = [figdir figfold 'hist_final_b_all'];
        %a(ind) = atand(tand(a(ind)+90))+90;
        C = 90-2.5+1.5;
    end
...
%         xpol(fd.ch(chind),j) = fd.aparam(chind,2);
    ...
    if i==1
        ind = ismember(ch(:,1),cutchans_a(inda==1));
        pname = [figdir figfold 'hist_final_a_all_mce0'];
        C = 0-2.5+1.5;
    else
        ind = ismember(ch(:,1),cutchans_b(indb==1)); %& ~any(isnan(a),2) ;%& any(abs(a)<4,2);
        pname = [figdir figfold 'hist_final_b_all_mce0'];
        %a(ind) = atand(tand(a(ind)+90))+90;
        C = 90-2.5+1.5;
    end
