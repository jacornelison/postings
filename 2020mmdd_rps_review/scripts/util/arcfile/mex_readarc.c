...
int mat_wrap_timestreams (struct dataset * ds, mxArray ** D)
{
  ...
  for (i=0; i<ds->nb; i++)
  {
    ...
    if (tmp == NULL)
      return -1;
    buflen = ((int64_t) numchan) * ds->buf[i].rb->spf * ds->buf[i].numframes * ds->buf[i].elsize;
    memcpy ((void *)mxGetPr(tmp), (void *)ds->buf[i].buf,buflen);
    mxSetField (board, 0, ds->buf[i].rb->regblock, tmp);
  }
  return 0;
}
...
void mexFunction (int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
    ...
    else if (r != 0)
    {
      ...
      /* mexErrMsgTxt ("Error opening or reading files.\n"); */
    }
    free_namelist (&(filt.nl));
