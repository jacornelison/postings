function d=arcvar_to_azel_ffflat(d)
% d=arcvar_to_azel(d)
%
...
d.horiz_mount_az=cvec(horiz_mount_az);
horiz_mount_el=repmat(horiz_mount(:,2)',samprate,1);
d.horiz_mount_el=cvec(horiz_mount_el);

% get encoder conversion factors (DK changed from bicep1 to bicep2)
az_mul= double(d.antenna0.tracker.encoder_mul(1,1))./360;
el_mul= -abs(double(d.antenna0.tracker.encoder_mul(1,2)))./360;
dk_mul= -abs(double(d.antenna0.tracker.encoder_mul(1,3)))./360;

% find actual x/y offset at each time step
d.eloff=double(d.antenna0.pmac.fast_el_pos)/el_mul-d.horiz_mount_el;
d.azoff=double(d.antenna0.pmac.fast_az_pos)/az_mul-d.horiz_mount_az;
ind=d.azoff<-180; d.azoff(ind)=d.azoff(ind)+360;

% rawpoint is just use crude conversion from fast rate register
d.rawpoint.hor.az=double(d.antenna0.pmac.fast_az_pos)/az_mul-cvec(repmat((double(d.antenna0.tracker.encoder_off(:,1))/3.6e6)',samprate,1));
d.rawpoint.hor.el=double(d.antenna0.pmac.fast_el_pos)/el_mul-cvec(repmat((double(d.antenna0.tracker.encoder_off(:,2))/3.6e6)',samprate,1));
d.rawpoint.hor.dk=double(d.antenna0.pmac.fast_dk_pos)/dk_mul-cvec(repmat((double(d.antenna0.tracker.encoder_off(:,3))/3.6e6)',samprate,1));
