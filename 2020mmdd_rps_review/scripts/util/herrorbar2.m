function hh = herrorbar(x, y, l, u, symbol,symbolcolor)
% 20140117 GPT
% hack to remove tees and use arbitrary colors
% best to specify symbolcolor vector
%
% For most purposes, use as follows or similar:
% 
% h=errorbar2(x,y,yerr,'k.'); % (draw y errors)
% hold on
% h=herrorbar2(x,y,xerr_low,xerr_high,'.',[0,0,0]); % (draw coincident x errors)
% hold off
%
...
tee=0; % additional hack to suppress tee
...
if ~exist('symbolcolor','var')
  h = plot(xb,yb,esymbol); hold on
  h = [h;plot(x,y,symbol)];
else
  h = plot(xb,yb,esymbol,'Color',symbolcolor,'MarkerFaceColor',symbolcolor); hold on
  h = [h;plot(x,y,symbol,'Color',symbolcolor,'MarkerFaceColor',symbolcolor)];
end
