...
int element_size (uint32_t typeword)
{
  ...
  switch (typeword & GCP_REG_TYPE)
  {
    ...
    default:
      printf ("Unknown register type 0x%x\n", typeword & GCP_REG_TYPE);
  }
...
int allocate_databuf (struct regblockspec * rb, struct chanlist * chan, int numframes, struct databuf * ts)
{
  ...
  if (ts->chan.n == 0)
    ts->bufsize = ((int64_t)numframes) * ts->elsize * rb->spf * rb->nchan;
  else
    ts->bufsize = ((int64_t)numframes) * ts->elsize * rb->spf * ts->chan.ntot;
...
int change_databuf_numframes (struct databuf * ts, int numframes)
{
  ...
  new_bufsize = ((int64_t)numframes) * ts->elsize * ts->rb->spf * numchan;
  ...
  new_chan_size = ((int64_t)numframes) * ts->rb->spf * ts->elsize;

  /* If zero buffer size, i.e. no channels, just change the maxframes */
  if (new_bufsize == 0)
  {
  ...
  else if (numframes > ts->maxframes)
  {
    new_ptr = realloc (ts->buf, new_bufsize);
    if (new_ptr == NULL) {
      fprintf (stderr, "realloc failed in %s.%s.%s.\n", ts->rb->map, ts->rb->board, ts->rb->regblock);
      return -1;
    }
...
int copy_to_buf (FILE * f, struct databuf * ts, int32_t * ofs)
{
  ...
  if (ts->numframes >= ts->maxframes) {
    printf("ts->numframes = %d, ts->maxframes = %d.\n", ts->numframes, ts->maxframes);
    return -1;
  }
