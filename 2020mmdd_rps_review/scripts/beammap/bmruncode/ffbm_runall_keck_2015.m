function ffbm_runall_keck_2015(bmnum,demod,fullmap,winmap,winmaplarge,...
                               html,plotmap)
% ffbm_runall_keck_2015(bmnum,demod,fullmap,winmap,winmaplarge,...
%                       html,plotmap)
%
...
% >> ffbm_runall_keck_2015(1,1,1,1,0,0); % For bm run 1 in 2015
%    Then modify the .csv field 'filename' with the timestamp found in
%    beammaps/maps (can do this with make_expt_year_bmcsv.m)
% >> ffbm_runall_keck_2015(1,0,0,0,1,1); % And the plots should be made,
%    with the HTML already in there!
% >> for ii = 2:47; ffbm_runall_keck_2015(ii,2,0,0,0,0); end; % Farm demods
...
switch demod
  ...
  case 2
    ...
    tmpfile = ['farmfiles/demod/k15demod_bm' sprintf('%02i',bmnum) '_' s];
    cmd = 'bm_demod(bmopt)';
...
switch fullmap
  ...
  case 2
    ...
    tmpfile = ['farmfiles/fullmap/k15fullmap_bm' sprintf('%02i',bmnum) '_' s];
    cmd = 'bm_makemap(bmopt)';
    farmit(tmpfile,cmd,'var',{'bmopt'},'queue',...
	'general,serial_requeue,itc_cluster,regal','mem',100000,'maxtime',1440);
end

% For 2015, the source position moved a bit after the mirror shift
bmopt.source_az = 0.3;
bmopt.source_el = 3.8;
if bmnum > 29
  bmopt.source_az = 0.1;
...
switch winmap
  ...
  case 2
    ...
    tmpfile = ['farmfiles/winmap/k15winmap_bm' sprintf('%02i',bmnum) '_' s];
    cmd = 'bm_makemap(bmopt)';
    farmit(tmpfile,cmd,'var',{'bmopt'},'queue',...
	'general,serial_requeue,itc_cluster,regal','mem',40000,'maxtime',1440);
end
...
switch winmaplarge
  ...
  case 2
    s = gen_stamp();
    mkdir('farmfiles/winmap')
    tmpfile = ['farmfiles/winmap/k15winmapl_bm' sprintf('%02i',bmnum) '_' s];
    cmd = 'bm_makemap(bmopt)';
    farmit(tmpfile,cmd,'var',{'bmopt'},'queue',...
	'general,serial_requeue,itc_cluster,regal','mem',40000,'maxtime',1440);
    bmopt.suffix = '';
...
switch plotmap
 ...
 case 2
  ...
  tmpfile = ['farmfiles/plotter/k15plotter_bm' sprintf('%02i',bmnum) '_' s];
